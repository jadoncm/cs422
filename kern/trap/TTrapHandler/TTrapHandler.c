...
{
    if(uctx_pool[get_curid()].trapno == T_PGFLT) pgflt_handler();
    else default_exception_handler();
    return;
}
...
{
	if(uctx_pool[get_curid()].trapno == IRQ_TIMER){
        	timer_intr_handler();
	}
  	else if(uctx_pool[get_curid()].trapno == IRQ_SPURIOUS){ 
